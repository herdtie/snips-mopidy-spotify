...
class HttpMopidyException(Exception):
    ...
    def __init__(self, error_params):
        """Create with content of field `error` of request response object or simple str"""
...
class HttpMopidy(object):
    ...
    def _post_request(self, method, **params):
        ...
        if self.debug:
            print('debug ch: method {} with params {} resulted in {}'.format(method, params, result.json()))
        result = result.json()
        if 'error' in result:
    ...
    def add_to_tracklist(self, uri):
        ...
        return self._post_request('core.tracklist.add', uri=uri)

    def play_single_track(self, uri):
        """Clears tracklist, adds given track, starts playing."""
        self.clear_tracklist()
        added = self.add_to_tracklist(uri)
        if not added:
            raise HttpMopidyException('No tracks added with {}'.format(uri))
        self.play()
